...
# Vercel will run this 'app' when a request comes to /api/badge
app = FastAPI()
...
def generate_svg_card(stats: dict):
    ...
    <svg width="400" height="165" xmlns="http://www.w3.org/2000/svg">
      <style>
        .container {{ font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; animation: fadeIn 0.8s ease-in-out; }}
      ...
      <g class="container" transform="translate(20, 20)">
        ...
        <g transform="translate(0, 50)"><text class="stat-label">Robo Rank</text><text y="25" class="stat-value">{rank}</text></g>
        <g transform="translate(140, 50)"><text class="stat-label">Robo Rating</text><text y="25" class="stat-value">{rating}</text></g>
        <g transform="translate(0, 100)">
          <text class="problems-solved">Problems Solved</text>
          <text x="360" y="0" text-anchor="end" class="stat-label">{solved} / {total}</text>
          <rect y="8" width="360" height="8" rx="4" fill="#30363d"/><rect y="8" width="{3.6 * progress_width}" height="8" rx="4" fill="#2e9a49"/>
        </g>
      </g>
    </svg>
    """
...
# This root endpoint now handles all GET requests to /api/badge
@app.get("/")
def get_badge(request: Request):
    """Returns a dynamic SVG stats card. The username is taken from the query parameters."""
    username = request.query_params.get('username')
    if not username:
        return Response("Missing 'username' query parameter", status_code=400)
